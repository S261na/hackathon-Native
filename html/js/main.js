...
// ---------------
const API = "http://localhost:8000/instruments";
...
let sectionInstruments = document.getElementById("sectionInstruments");
let searchValue = "";
...
btnAdd.addEventListener("click", () => {
  ...
  let newInstr = {
    instrName: inpName.value,
    instrType: inpType.value,
    instrImage: inpImage.value,
    instrPrice: inpPrice.value,
  };
  createInstr(newInstr);
  readInstr();
});

// CREAT
function createInstr(instruments) {
  fetch(API, {
    method: "POST",
    headers: {
...
function readInstr() {
  fetch(`${API}?q=${searchValue}&_page=${curentPage}&_limit=6`)
    .then((res) => res.json())
    .then((data) => {
      sectionInstruments.innerHTML = "";
      data.forEach((item) => {
        sectionInstruments.innerHTML += `
         <div class = "card m-4 cardBook"
         style = "width: 18rem"> 
        <img id = "${item.id}" src = "${item.instrImage}" 
        class = "card-img-top detailsCard" style = "height: 280px" alt = "${item.instrName}" />
        <div class = "card-body">
            <h5 class = "card-title">${item.instrName}</h5>
            <p class = "card-text">
            ${item.instrType}
            </p>
            <p><strong>${item.instrPrice}</strong></p>
              <button class = "btn btn-outline-success btnDelete" id = "${item.id}">Delete</button>
...
let editInpType = document.getElementById("editInpAuthor");
  ...
  if (arr.includes("btnEdit")) {
    ...
    fetch(`${API}/${id}`)
      ...
      .then((data) => {
        ...
        editInpName.value = data.instrName;
        editInpType.value = data.instrAuthor;
        editInpImage.value = data.instrImage;
        editInpPrice.value = data.instrPrice;
...
editBtnSave.addEventListener("click", () => {
  let editedInstr = {
    instrName: editInpName.value,
    instrAuthor: editInpType.value,
    instrImage: editInpImage.value,
    instrPrice: editInpPrice.value,
  };
  editInstr(editedInstr, editBtnSave.id);
});
// Функция для отправки обновленных данных на сервер
function editInstr(editedInstr, id) {
  fetch(`${API}/${id}`, {
    method: "PATCH", // служат для редактирования элементов, позволяет заменять частично
    headers: {
      "Content-type": "application/json;charset=utf-8",
    },
    body: JSON.stringify(editedInstr),
  }).then(() => readInstr());
}
...
function pageFunc() {
  fetch(`${API}?q=${searchValue}`)
    .then((res) => res.json())
    .then((data) => {
      countPage = Math.ceil(data.length / 6); // округляем товары до 6 элементов, от сова потолок, будет округлять до верхнего целого числа, 5.9 = 6
    });
}
...
nextBtn.addEventListener("click", () => {
  if (curentPage >= countPage) return;
  curentPage++;
  readInstr();
});
...
// await axios.get(API) - запрос на сервер на реакте
